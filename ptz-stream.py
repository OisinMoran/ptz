...
def index():
    options = {
        49: "Orange (down with scurvy)",
        0: "Person (a matey)",
        8: "Boat (cannons at the ready)",
        39: "Bottle (perhaps a message inside?)",
    }
    ...
    <body>
        <h1>Live Vitamin Sea Tracker</h1>
        <form method="post" action="/set_class">
            <label for="class_id">Select item to track:</label>
            <select name="class_id" onchange="this.form.submit()">
                {% for id, label in options.items() %}
                    <option value="{{ id }}" {% if id == tracked_class %}selected{% endif %}>
                        {{ label }}
                    </option>
                {% endfor %}
            </select>
...
def set_class():
    global TRACKED_CLASS
    TRACKED_CLASS = int(request.form["class_id"])
    return redirect("/")
...
while True:
    ...
    if model_success or tracker_success:
        ...
        zoom = 0.8 * prev_zoom + 0.2 * min(
            # Try to keep the item at 40% of width or height (smaller)
            max(0.4 * min(IMG_WIDTH / w, IMG_HEIGHT / h), 1),
            10,
        )
